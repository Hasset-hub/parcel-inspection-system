...
class DamageDetection(Base):
    ...
    # Foreign keys
    inspection_id = Column(UUID(as_uuid=True), ForeignKey('inspections.inspection_id', ondelete='CASCADE'), nullable=False)
    ...
    # Relationships - FIXED: back_populates must match Inspection's relationship name
    inspection = relationship("Inspection", back_populates="damage_detections")
    
    def __repr__(self):
        return f"<DamageDetection {self.damage_type} ({self.confidence:.2f})>"
